...
import com.xk.exception.ExceptionCode;
import com.xk.exception.HeraIllegalArgumentException;
        ...
        try {
            if (StringUtils.isNotEmpty(jsonschema)) {
                ...
                if (!report.isSuccess()) {
                    ...
                    throw new HeraIllegalArgumentException(ExceptionCode.SYSTEM_ILLEGAL_REQUEST_B);
                }
            } else {
                log.debug("====jsonSchemaValidation jsonSchema is null====");
            }
        } catch (HeraIllegalArgumentException e) {
            throw e;
        } catch (Exception e) {
